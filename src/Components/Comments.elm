                            ...
                            , div [ class "dropdown is-right" ]
                                ...
                                , div [ id ("edit-ellipsis-" ++ c.id), class "dropdown-menu", attribute "role" "menu" ]
                                    [ div [ class "dropdown-content p-0" ] <|
                                        [ div [ class "dropdown-item", attribute "data-clipboard" c.message, onClick (OnCopyLink c.id) ] [ A.icon1 "icon-copy" T.copyText ]
                                        , div [ class "dropdown-item", attribute "data-clipboard" reflink, onClick (OnCopyLink c.id) ] [ A.icon1 "icon-link" T.copyLink ]
                                        ]
                                            ++ (if isAuthor then
...
viewCommentInputHeader op targetid form =
    ...
    div [ class "level commentHeader" ]
        ...
        , div [ class "level-right is-hidden-mobile" ]
            [ div [ onClick (op.onRichText targetid "Heading"), title T.headingText ] [ text "H" ]
            , div [ onClick (op.onRichText targetid "Bold"), title T.boldText ] [ strong [] [ text "B" ] ]
            , div [ onClick (op.onRichText targetid "Italic"), title T.italicText ] [ span [ class "is-italic" ] [ text "I" ] ]
            , div [ onClick (op.onRichText targetid "Strikethrough"), title T.strikethroughText ] [ span [] [ text ("̶" ++ "S" ++ "̶") ] ]
            , div [ onClick (op.onRichText targetid "Quote"), class "mr-3", title T.quoteText ] [ span [] [ A.icon "icon-quote-right icon-xs" ] ]
            , div [ onClick (op.onRichText targetid "Link"), title T.linkText ] [ span [] [ A.icon "icon-link icon-sm" ] ]
            , div [ onClick (op.onRichText targetid "List-ul"), title T.listText ] [ span [] [ A.icon "icon-list-ul icon-sm" ] ]
            , div [ onClick (op.onRichText targetid "List-ol"), title T.orderedListText ] [ span [] [ A.icon "icon-list-ol icon-sm" ] ]
            , div [ onClick (op.onRichText targetid "List-check"), title T.checkListText ] [ span [] [ A.icon "icon-check-square icon-sm" ] ]
            , div [ onClick (op.onRichText targetid "Details"), class "mr-3", title T.collapsibleSectionText ] [ A.icon "icon-chevron-right icon-sm" ]
            , div [ onClick (op.onRichText targetid "MentionUser"), title T.mentionUserText ] [ span [] [ A.icon "icon-at-sign icon-sm" ] ]
            , div [ onClick (op.onRichText targetid "MentionTension"), class "mr-3", title T.mentionTensionText ] [ A.icon "icon-exchange icon-sm" ]
            , div
